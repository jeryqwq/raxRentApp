      ...
      <Form scrollToFirstError onSubmit={async (values, error) => {
        if(values) {
          const res = await myRequest({
            ...
            method: 'post',
            data: {isNew: 1,...values, id: uuid, equipType: values.equipType[values.equipType.length - 1], releaseCityName:  Array.isArray(values.releaseCityName) ? values.releaseCityName.join(',') : values.releaseCityName}
          })
          showToast({
            type: 'success',
            content: '新增成功!'
